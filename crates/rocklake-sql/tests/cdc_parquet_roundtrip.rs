...
async fn cdc_parquet_roundtrip_multi_file_window() {
    ...
    let inserts: Vec<_> = result
        .records
        .iter()
        .filter(|r| r.change_type == rocklake_sql::ChangeType::Insert)
        .collect();
    let deletes: Vec<_> = result
        .records
        .iter()
        .filter(|r| r.change_type == rocklake_sql::ChangeType::Delete)
        .collect();
...
async fn cdc_parquet_notfound_returns_storage_error() {
    ...
    assert!(
        matches!(err, rocklake_sql::TableChangesError::Storage(_)),
...
async fn cdc_record_count_mismatch_increments_counter() {
    ...
    let before = rocklake_sql::cdc_record_count_mismatch_total();
    ...
    let after = rocklake_sql::cdc_record_count_mismatch_total();
    assert!(
        after > before,
