...
SlateDuck does not need any of that complexity. The DuckLake protocol defines a specific, finite set of SQL operations that a catalog client will ever need to perform — and SlateDuck implements exactly that set, no more.
...
These are handled by pre-configured responses. `version()` returns a PostgreSQL-compatible version string. `pg_type` queries return the type OIDs that DuckDB expects for data type resolution. `SET` statements for known parameters are acknowledged and discarded (SlateDuck does not use a session timezone internally).
...
Data file queries are the most performance-critical catalog operation — every table scan triggers at least one. SlateDuck optimizes these with a secondary index (tag `0xFC`) that makes snapshot-scoped file lookups fast even for tables with thousands of files.
...
The explicit non-support is as important as what is supported. These are the most common queries that might arrive at SlateDuck and be rejected:
...
SlateDuck does not implement SQL joins. This query would be rejected with `SQLSTATE 0A000`. DuckDB never issues this query — it asks for tables and columns separately and performs the join client-side.
...
No subqueries, no CTEs. Every query that SlateDuck handles is a flat statement against a single table or a pre-enumerated set of tables.
...
Rows are never deleted in SlateDuck's catalog model. Instead, rows are superseded by setting their `end_snapshot`. This preserves the complete history (immutability) and makes crash recovery trivially safe (no deletion to roll back). A `DELETE` statement arriving at SlateDuck is a bug in the client — DuckDB's `ducklake` extension never issues them.

**User-defined functions, stored procedures, triggers:**

None of these exist in SlateDuck's surface area. There is no procedural language, no function registry, and no trigger mechanism. The catalog is a key-value store with a SQL interface, not a programmable database.

## How Classification Works

When a SQL string arrives over the PostgreSQL wire protocol, SlateDuck processes it in three stages:

**Parsing.** The raw SQL text is parsed by `sqlparser-rs` using the PostgreSQL dialect. This produces an AST. If the SQL is syntactically invalid, `sqlparser-rs` returns an error and SlateDuck responds with `SQLSTATE 42601` (syntax error) before any classification attempt.
...
If you are operating SlateDuck as a DuckLake catalog backend for DuckDB, you will never encounter the bounded-SQL limitation in normal usage — DuckDB's `ducklake` extension issues only the statements that are in the bounded set by design. The limitation only becomes visible if you:

1. **Connect to SlateDuck directly with a PostgreSQL client** (like `psql`) and try to run arbitrary SQL. You will find that most queries work for catalog inspection purposes (reading `ducklake_table`, `ducklake_column`, etc.) but more complex queries fail.
...
3. **Use a DuckDB version newer than SlateDuck supports.** If DuckDB's `ducklake` extension adds a new catalog operation that SlateDuck does not yet recognize, queries that trigger the new operation will fail with `SQLSTATE 0A000`. This surfaces as a DuckDB error like "unsupported catalog operation." Check the compatibility matrix in [DuckDB Compatibility](../integration/duckdb-compatibility.md).

For all three cases, the appropriate response is the same: understand what statement SlateDuck is rejecting, determine whether it is something SlateDuck should support, and either file an issue or adjust your client accordingly.

## Extending the Bounded Set

When a new version of DuckDB's `ducklake` extension introduces new catalog operations, SlateDuck must be updated to recognize the new statement shapes. This is a deliberate design choice: compatibility is explicit, not implicit. Each new statement shape requires:

1. A new variant in the `StatementKind` enum in `slateduck-sql/src/lib.rs`
...
This means that SlateDuck's compatibility with DuckDB versions is well-defined and testable. You can look at the `StatementKind` enum and know exactly which DuckDB `ducklake` extension operations are supported. You can run the wire corpus replay tests against a specific DuckDB version's corpus and know whether SlateDuck handles it correctly. Compatibility is never ambiguous.
...
- **[DuckDB Compatibility](../integration/duckdb-compatibility.md)** — Which DuckDB versions work with which SlateDuck versions
- **[Custom Clients](../integration/custom-clients.md)** — How to build a new DuckLake client that works with SlateDuck
